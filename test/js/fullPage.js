...
function scrollBoxDetect(pageCount) {
  const $scrollBox = $pages[pageCount].querySelector('.fp_scroll_box');
  if(!$scrollBox) return;
  let scrollPos = Math.round($scrollBox.scrollTop);
  let scrollBottom = $scrollBox.scrollHeight - $scrollBox.clientHeight;

  document.querySelector('.test1').innerText = `scrollTop : ${scrollPos}`;
  document.querySelector('.test2').innerText = `scrollHeight - clientHeight : ${scrollBottom}`;

  if(scrollBottom === 0) {
    return 'null'; 
  }

  if(scrollBottom !== 0 && scrollPos === 0) {
    return 'top';

  } else if(scrollBottom !== 0 && scrollPos === scrollBottom) {
    return 'bottom';
...
function wheelUp() {
  ...
  if(pageCount > 0) {
    if($pages[pageCount].querySelector('.fp_scroll_box')) {
