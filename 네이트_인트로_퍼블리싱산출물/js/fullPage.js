...
export const $menus = document.querySelectorAll('.fp_menu');
export const $moveBtns = document.querySelectorAll('.fp_move_btn');
export const $topBtn = document.querySelector('.fp_top_btn');
export const $navWrap = document.querySelector('.fp_nav_wrap');
export const $navs = document.querySelectorAll('.fp_nav');
export const pageWrapTime = Number(getComputedStyle($pageWrap).transition.replace(/([^0-9.])/g, '') * 1000);
export const pageLength = $pages.length;
...
if ($menuWrap) {
  if (!Number($menus[0].dataset.fpnum) && Number($menus[0].dataset.fpnum) !== 0) {
    const num = $menus[0].dataset.fpnum
      .replaceAll(',', '')
      .split(' ')
      .map((elm) => Number(elm))
      .sort((a, b) => a - b)[0];
...
function valueReset() {
  ...
  let numFlag = false;
  $menus.forEach((elm) => {
    const menuArr = elm.dataset.fpnum
      .replaceAll(',', '')
      .split(' ')
      .map((elm) => Number(elm))
      .sort();
...
// move btn click event 250625 최적화 안됨
function moveBtnsClickEvent(btn) {
  topDetect = false;
  bottomDetect = false;
  if (topBtnFlag || menuFlag || navFlag || specificFlag || moveBtnFlag) return;
  let btnNum = Number(btn.dataset.fpnum);
  if (!btnNum) return;
  moveBtnFlag = true;
  pagePos = 0;
  if (btnNum === 0) {
  ...
  topBtnOnOff(pageCount, pageLength);
  $pageWrap.style.transform = `translateY(-${pagePos}px)`;
  setTimeout(() => (moveBtnFlag = false), pageWrapTime);
}
...
$moveBtns.forEach((elm) => {
  if (elm) elm.addEventListener('click', () => moveBtnsClickEvent(elm));
});
