...
window.addEventListener('load', () => {
  ...
  // page element
  const $contactUsSelectBox = document.querySelector('.contactUs #select-box1');
  ...
  window.addEventListener('click', (e) =>{
    ...
    if($contactUs) {
      if(!e.target.closest('#select-box1') && $contactUsSelectBox.classList.contains('on')) {
        $contactUsSelectBox.classList.remove('on');
      }
    }
  });
