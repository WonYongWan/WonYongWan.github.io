...
window.addEventListener('load', () => {
  ...
  if($sub4) {
    ...
    let fileListIdx = sessionStorage.getItem('fileListIdx') ? Number(sessionStorage.getItem('fileListIdx')) : 0;
    ...
    if($inputFile1) {
      ...
      $inputFile1.addEventListener('change', () => {
        for(let i = 0; i < $inputFile1.files.length; i++) {
          if($inputFile1.files[i].type === imageType[$inputFile1.files[i].type] && !fileData[$inputFile1.files[i].name]) {

            if((fileTotalSize + ($inputFile1.files[i].size * 0.000001)) >= 10) {
              alert('파일용량이 10MB를 초과 하였습니다.');
              return
            } else {
